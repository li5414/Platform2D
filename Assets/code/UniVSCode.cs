...
public class UniVSCode : MonoBehaviour
{
    ...
    //     "**/**.cs.meta": true
    // }
    [MenuItem("File/Use VSCode", true)]
    static bool ValidateUncheckVSCode()
    {
        bool state = UseVSCode();
        Menu.SetChecked("File/Use VSCode", state);
        return IsOnAMac();
    }

    [MenuItem("File/Use VSCode")]
    ...
    {
        UnityEngine.Debug.Log( "[VSCODE open] " + ProjectPath());
    ...
    static bool OnOpenedAssetCallback(int instanceID, int line)
    {
        ...
        if (selected.GetType().ToString() == "UnityEditor.MonoScript")
        {
            ...
            if (line == -1)
            {
                args = " -n -b \"com.microsoft.VSCode\" --args -r \"" + completeFilepath + "\" -r";
            }
            else
            {
                args = " -n -b \"com.microsoft.VSCode\" --args -r -g \"" + completeFilepath + ":" + line.ToString() + "\" -r";
            }
