...
namespace druggedcode.engine
{
	...
	public class DEActor : MonoBehaviour
	{
		...
		{
			if (AllActor.Contains (ch) == false)
				AllActor.Add (ch);
		}

		static private void Unregister (DEActor ch)
		{
			if (AllActor.Contains (ch))
				AllActor.Remove (ch);
		}
		...
		protected void SpawnAtFoot (GameObject prefab, Quaternion rotation, Vector3 scale)
		{
			...
			if (footEffectBone != null)
			{
				...
				if (bone != null)
					pos = graphic.TransformPoint (bone.WorldX, bone.WorldY, 0f);
			}
		...
		protected void PlatformSoundPlay ()
		{
			...
			if (platform != null)
				platform.PlaySound (mTr.position, 1f, 1f);
		}
		...
		protected void PlatformEffectSpawn ()
		{
			Platform platform = Controller.State.StandingPlatform;
			if (platform != null)
				platform.ShowEffect (mTr.position, new Vector3 (mFacing * 1f, 1f, 1f));
		}
		...
		protected void SetFacing (int facing)
		{
			...
			if (mFacing == 1) mSkeletonAnimation.Skeleton.FlipX = mAnimFlip;
		...
		virtual protected void Idle ()
		{
			...
			if (Controller.State.PlatformVelocity == Vector2.zero)
